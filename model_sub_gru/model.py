...
try:
    ...
    _HAS_TORCH = False
    raise ImportError("PyTorch is required for GRU model. Install with: pip install torch")
...
class ImprovedGRUModel(nn.Module):
    def __init__(self, input_size=4, hidden_size=128, num_layers=3, dropout=0.3, output_size=14):
        super(ImprovedGRUModel, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        
        # Bidirectional GRU with more capacity
        self.gru = nn.GRU(
            input_size, 
            hidden_size, 
    ...
    def forward(self, x):
        # GRU forward pass (bidirectional gives 2x hidden_size)
        gru_out, _ = self.gru(x)  # Shape: (batch, seq_len, hidden_size * 2)
        
        # Attention mechanism
        attn_out, _ = self.attention(gru_out, gru_out, gru_out)  # Shape: (batch, seq_len, hidden_size * 2)
        
        # Combine last output with attended output
        last_gru = gru_out[:, -1, :]  # Last GRU output
        last_attn = attn_out[:, -1, :]  # Last attended output
        combined = (last_gru + last_attn) / 2  # Average combination
    ...
    for stn, grp in daily[daily["station_name"].isin(stations)].groupby("station_name"):
        ...
        for i in range(len(grp) - HIST_DAYS - FUTURE_DAYS + 1):
            ...
            if hist_block.isna().any().any():
                continue
            # For GRU, we keep the sequence shape (7, 4) instead of flattening
...
def main():
    ...
    try:
        ...
        if model_pkl_path.exists():
            ...
            gru_model = model_dict['model_class'](
            ...
            gru_model.load_state_dict(model_dict['model_state_dict'], strict=False)
            ...
            # Initialize GRU model with improved architecture
            gru_model = ImprovedGRUModel(input_size=len(FEATURES), hidden_size=128, num_layers=3, dropout=0.3, output_size=14)

            criterion = nn.BCELoss()
            optimizer = optim.Adam(gru_model.parameters(), lr=0.001, weight_decay=1e-5)
        ...
        gru_model.eval()
        with torch.no_grad():
            y_pred_probs = gru_model(X_te_tensor).numpy()
